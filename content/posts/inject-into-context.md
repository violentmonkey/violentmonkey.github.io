...
The default mode is [`auto`](#mode-auto), you can change it in Violentmonkey's settings. The actual mode can be seen by a userscript in [GM_info.injectInto](/api/#gm_info), also `content` mode scripts in Violentmonkey v2.42.0 or newer have a `content` text shown in their `...` menu in the popup, clicking the text opens this article.
