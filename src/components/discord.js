...
export default function DiscordButton() {
  ...
  return open
      ...
      >
        <div className="p-2 cursor-pointer" onClick={() => setOpen(false)}>
          <svg viewBox="0 0 12 12" className="w-4 h-4">
            <path d="M0 1L11 12L12 11L1 0zM11 0L0 11L1 12L12 1z" stroke="none" fill="currentColor" />
          </svg>
        </div>
        <iframe
    ...
    : (
      <div className="fixed bottom-0 right-8 px-8 py-2 bg-blue-500 text-white rounded-t-lg uppercase cursor-pointer" onClick={() => setOpen(true)}>
        Open Chat
      </div>
    );
}
